"""
JobGuard Core Compliance - Statutory Labor Penalties Master Catalog
Contains 200 comprehensive legal statutes, penalty ceilings, custodial sentencing limits,
...
@dataclass
class MasterStatutoryLaborPenalty:
    statute_id: str
    country_code: str
...
STATUTORY_LABOR_PENALTIES_DATA: List[MasterStatutoryLaborPenalty] = [
    MasterStatutoryLaborPenalty(
        statute_id="LAW-PEN-IN-0001",
        country_code="IN",
...
class StatutoryLaborPenaltiesManager:
    """Manager class for querying statutory labor penalties."""

    def __init__(self):
        self.penalties = {p.statute_id: p for p in STATUTORY_LABOR_PENALTIES_DATA}
        self.country_index: Dict[str, List[MasterStatutoryLaborPenalty]] = {}
        for p in STATUTORY_LABOR_PENALTIES_DATA:
            c = p.country_code.upper()
            if c not in self.country_index:
                self.country_index[c] = []
            self.country_index[c].append(p)

    def get_by_id(self, statute_id: str) -> Optional[MasterStatutoryLaborPenalty]:
        return self.penalties.get(statute_id.strip().upper())

    def get_by_country(self, country_code: str) -> List[MasterStatutoryLaborPenalty]:
        return self.country_index.get(country_code.strip().upper(), [])
